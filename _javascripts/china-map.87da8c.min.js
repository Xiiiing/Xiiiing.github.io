const commonChartOptions={tooltip:{trigger:'item',formatter:'{b}'},series:[{type:'map',roam:true,selectedMode:false,label:{show:true,formatter:'{b}',textStyle:{color:'#333',fontSize:10}},itemStyle:{normal:{areaColor:'#e6f7ff',borderColor:'#91d5ff',borderWidth:1},emphasis:{areaColor:'#91d5ff',label:{show:true,textStyle:{color:'#000',fontSize:12}}}}}]};let currentMap='china';let chartInstance=null;const provinceMapConfig={'辽宁':{mapName:'liaoning',jsonPath:'/_javascripts/21.json',seriesName:'辽宁地图'},'四川':{mapName:'sichuan',jsonPath:'/_javascripts/51.json',seriesName:'四川地图'},'广东':{mapName:'guangdong',jsonPath:'/_javascripts/44.json',seriesName:'广东地图'}};function initMap({selector,mapName,jsonPath,seriesName,data,onClick}){if(chartInstance){chartInstance.dispose();chartInstance=null;}
const chartDom=document.querySelector(selector);if(!chartDom)return;const chart=echarts.init(chartDom);chartInstance=chart;fetch(jsonPath).then(res=>res.json()).then(geoJson=>{echarts.registerMap(mapName,geoJson);const option={title:{text:seriesName,left:'center',top:'20px',textStyle:{fontSize:18,fontWeight:'bold'}},...commonChartOptions,series:[{...commonChartOptions.series[0],name:seriesName,map:mapName,data:data||[]}]};chart.setOption(option);if(onClick){chart.on('click',params=>{if(params.componentType==='series'&&params.seriesType==='map'){onClick(params.name);}});}}).catch(err=>console.error(`${seriesName}地图加载失败:`,err));}
