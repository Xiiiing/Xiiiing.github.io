...
const container=document.querySelector('#map-container');const chartInstance=echarts.init(container);const canvas=chartInstance.getZr().canvas;canvas.style.zIndex=10;if(!container)return;const chart=echarts.init(container);chartInstance=chart;try{const res=await fetch(jsonPath);const geoJson=await res.json();echarts.registerMap(mapName,geoJson);const option={...commonChartOptions,series:[{...commonChartOptions.series[0],name:seriesName,map:mapName,data:data.map(item=>({...item,itemStyle:{normal:{areaColor:'#1890ff'},emphasis:{areaColor:'#40a9ff'}}}))}]};chart.setOption(option);chart.off('click');chart.on('click',(params)=>{if(params.componentType==='series'){handleMapToggle(params.name);}});}catch(err){console.error(`${seriesName} 地图加载失败:`,err);}};function handleMapToggle(name){if(currentMap==='china'&&provinceMapConfig[name]){const{mapName,jsonPath,seriesName}=provinceMapConfig[name];currentMap=name;initMap({mapName,jsonPath,seriesName,data:highlightData[name]});}else if(currentMap==='china'){showAutoCloseDialog(`笔者未曾求学于 <strong>${name}</strong>`);}else{currentMap='china';initMap({mapName:'china',jsonPath:'/_javascripts/china.json',seriesName:'中国地图',data:highlightData.china});}}
initMap({mapName:'china',jsonPath:'/_javascripts/china.json',seriesName:'中国地图',data:highlightData.china});function showAutoCloseDialog(message,duration=2000){const overlay=document.createElement('div');overlay.style.cssText=`
    position: fixed;
    top: 20px;
    left: 50%;
    ...
    text-align: center;
    line-height: 1.5;
  `;overlay.innerHTML=message;document.body.appendChild(overlay);setTimeout(()=>{document.body.removeChild(overlay);},duration);}