...
window.addEventListener('load', function() {
    // 假设china-map.js提供initMap函数，参数为容器ID和配置项
    if (typeof initMap === 'function') {
       initMap({ mapName: 'china', jsonPath: 'china.json', seriesName: '中国地图', data: highlightData.china });
    } else {
        console.error('china-map.js未正确加载或缺少initMap函数');
    }
});
/*
...
const provinceMapConfig = {
  '辽宁省': { mapName: 'liaoning', jsonPath: '21.json', seriesName: '辽宁地图' },
  '四川省': { mapName: 'sichuan',  jsonPath: '51.json', seriesName: '四川地图' },
  '广东省': { mapName: 'guangdong',jsonPath: '44.json', seriesName: '广东地图' }
};
